...
2. **المعالجة المسبقة والتدريب.** تستدعي أغلفة Shell البرنامجين الأصليين `pre_processing.py` و`generate_graph.py`، ثم `train.py` من هذا المستودع للتدريب بعملية واحدة. واستخدم تشغيل المرحلة الثانية كامل البيانات إعداد مهمة Condor في المستودع لإطلاق عملية واحدة لكل GPU. يمكن أن يبدأ التدريب عشوائيًا ببروتوكول التسجيل ثم الإرساء ذي المرحلتين، أو أن يضبط الأوزان المنشورة ضبطًا دقيقًا.
