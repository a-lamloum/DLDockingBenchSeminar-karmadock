window.KD_SEARCH_INDEX = {
  "entries": [
    ...
    {
      "excerpt": "A plain-English guide to every meaningful prototype, shared, and full-data file in the final KarmaDock submission, grouped by directory with links to code walkthroughs.",
      "heading": "File-by-file guide",
      "text": "reference file-by-file guide repository root readme.md final submission report: full-data headline 82.2 % vs released 88.3 %, three pipelines, full-data results with prototype context, reproduction, training parameters, repository layout, issues. dockerfile reproducible image ahlamloum/karmadock-seminar:v6: miniconda, upstream karmadock clone, authors' pre-packed conda env zenodo tarball, prefetch_generator, rmsd, wandb, scripts/, pythonpath, build-time import check. .gitignore ignores prototype data bundle, __pycache__, notebook checkpoints, local data/ symlinks and keeps tracked *.csv splits. scripts/train.py main checkpointed two-stage trainer with mdn and rmsd losses, optimizer, early stopping, gradient accumulation, deterministic val split, w&b, --resume. scripts/train_ddp.py full data multi-gpu ddp stage-2 trainer, 2× a100, torchrun, nccl, id-shard, all-reduced metrics, rank-0 checkpointing. scripts/run_train.sh prototype scratch p2 and finetune p3 driver, proto_train 712. scripts/run_full_train.sh full data driver, full_train 23,483, full_val 2,609, --val_csv, --val_graph_dir. scripts/run_full_stage2_ddp.sh full data torchrun launcher. scripts/run_full_stage2_ddp_v2.sh full data paper-faithful weight_decay 0 patience 70 final 2× a100 launcher. scripts/run_infer.sh preprocess dock export uncorrected ff align. scripts/evaluate.sh evaluation.py for each pipeline and pose variant, rmsd csvs. scripts/convert_seminar_to_karmadock.py seminar data to karmadock layout. scripts/convert_karmadock_to_seminar.py docked poses to <id>_pred.sdf best-pose-first. scripts/seminar_csv.py csv-schema helper. scripts/readme.md team vs upstream provenance. condor/p1_baseline.sub condor/p2_scratch_infer.sub condor/p3_finetune_infer.sub prototype inference and evaluation. condor/p2_train_scratch.sub condor/p3_finetune.sub prototype training. condor/evaluate.sub prototype scoring. condor/full_train_scratch.sub full data stage-1. condor/full_stage2_2gpu.sub full data 2× a100 ddp submission model. condor/full_stage2_mgpu.sub full data multi-gpu test. condor/smoke_2gpu.sub full data ddp smoke test. condor/full_test_infer.sub condor/posebusters_infer.sub full data inference. condor/full_eval_batch.sub full data 42-job sharded evaluation. condor/full_eval_jobs/ full data run.sh per job with --shard_idx and --num_shards. condor/logs/s2_2gpu.169253.* full data completed stage-2 training exit 0 epoch 469. condor/logs/full_test_infer.* posebusters_infer.* released_baseline counterparts full-data inference logs. condor/logs/full_data_eval/ full data 42 evaluation .log .err .out files. data/proto_test.csv prototype 136-complex mapping. data/full_test.csv full data 6,183-complex mapping. data/posebusters_filtered.csv full data 308-complex posebusters subset. data/prototype_model_data.zip prototype crystal structures for local scoring. docs/p2_stage1_train_log.csv docs/p2_stage2_train_log.csv docs/p3_finetune_train_log.csv prototype per-epoch curves. docs/full_stage2_train_log.csv full data stage-2 ddp curve. docs/full_stage2_training_curve.png full data training figure. docs/workflow_training.png docs/workflow_inference.png site workflow diagrams. evaluation/evaluation.py seminar evaluator with upstream logic unmodified, team posebusters validity check, --shard_idx and --num_shards default no-op sharding across 42 jobs. model/full_scratch_karmadock_team002.pkl full data final submission model behind 82.2 %. model/p2_scratch_karmadock_team002.pkl prototype p2 from-scratch checkpoint proto_train 712. model/p3_finetune_karmadock_team002.pkl prototype p3 fine-tune bonus. notebooks/full_data_results_and_comparison.ipynb full data final notebook with tables, ecdf, posebusters failure modes, paper comparison. notebooks/prototype_results_and_comparison.ipynb prototype notebook proto_test 136. results/full_data_evaluation/ full data per-complex rmsd and posebusters csvs for full_scratch and released_baseline across full_test and posebusters and three variants, ecdf_full_data.png, read by full-data notebook. results/proto_test/ results/{p1_baseline,p2_scratch,p3_finetune}/proto_test{,_ff,_align}/ prototype poses for three pipelines, three variants, 136 complexes. results/*_evaluation.csv results/readme.md prototype rmsd csvs and results readme.",
      "title": "Reference",
